...
# print(f"user attempts: {greeting.login_attempts}")
class Admin (User):
   def __init__(self, first_name, last_name, year_level, salary, privileges):
       super().__init__(first_name, last_name, year_level, salary)
       self.first_name = first_name
       self.last_name = last_name
       self.year_level = year_level
       self.salary = salary
       self.pricvileges = privileges
   def  show_privileges(self) :
       print ('privileges can:')
       for privilege in self.pricvileges:
           print(f"-{privilege}")
pricvileges = Admin ('Eman','shaouq','final','2000',['can add post','delete post','give u zero'])

pricvileges.show_privileges()
